...
bool WinStreamHostWrapper::loadAudioFile(string wavname)
{
...
Vamp::Plugin* WinStreamHostWrapper::createPlugin(string myname, string soname,
...
void WinStreamHostWrapper::usage(const char *name)
{
	COUT(
...
int WinStreamHostWrapper::shellMain(int argc, char **argv)
{
	char *scooter = argv[0];
	char *name = 0;
	...
	if (argc == 2)
	{
		...
		if (!strcmp(argv[1], "-v"))
		{
			...
			COUT(
					"Simple Vamp plugin host version: " << HOST_VERSION << endl << "Vamp API version: " << VAMP_API_VERSION << endl << "Vamp SDK version: " << VAMP_SDK_VERSION << endl);
			return 0;
	...
	loadPlugin(str, soname, plugid, output, outputNo, useFrames, 44100);

	// The -1 is to avoid dealing with the partial last block.
	for (word wBlockIndex = 0;
			wBlockIndex < (wavFileLoader.GetSize() / (m_dwBlockSize * sizeof(word))) - 1;
			wBlockIndex++)
	{
		runPlugin(
				(word*) (wavFileLoader.GetData()) + (wBlockIndex * m_dwBlockSize),
				(m_dwBlockSize * sizeof(word)));
	}
...
WinStreamHostWrapper::~WinStreamHostWrapper()
{

}

void WinStreamHostWrapper::printPluginPath(bool verbose)
{
	if (verbose)
	{
...
string WinStreamHostWrapper::header(string text, int level)
{
	string out = '\n' + text + '\n';
...
void WinStreamHostWrapper::enumeratePlugins(Verbosity verbosity)
{
	PluginLoader *loader = PluginLoader::getInstance();
			...
			++i)
	{
		...
		if (plugin)
		{
			...
			if (verbosity == PluginInformation)
			{
				...
				COUT(
						"    [" << c << "] [v" << plugin->getVampApiVersion() << "] " << plugin->getName() << ", \"" << plugin->getIdentifier() << "\"" << " [" << plugin->getMaker() << "]" << endl);
			...
			else if (verbosity == PluginInformationDetailed)
			{
				...
				COUT(
						" - Plugin Version:     " << plugin->getPluginVersion() << endl);
				COUT(
						" - Vamp API Version:   " << plugin->getVampApiVersion() << endl);
				COUT(
						" - Maker:              \"" << plugin->getMaker() << "\"" << endl);
				COUT(
						" - Copyright:          \"" << plugin->getCopyright() << "\"" << endl);
				COUT(
						" - Description:        \"" << plugin->getDescription() << "\"" << endl);
				COUT(
						" - Input Domain:       " << (plugin->getInputDomain() == Vamp::Plugin::TimeDomain ? "Time Domain" : "Frequency Domain") << endl);
				COUT(
						" - Default Step Size:  " << plugin->getPreferredStepSize() << endl);
				COUT(
						" - Default Block Size: " << plugin->getPreferredBlockSize() << endl);
				COUT(
						" - Minimum Channels:   " << plugin->getMinChannelCount() << endl);
				COUT(
						" - Maximum Channels:   " << plugin->getMaxChannelCount() << endl);
			...
			if (verbosity == PluginInformationDetailed)
			{
				...
				for (size_t j = 0; j < params.size(); ++j)
				{
					...
					COUT(
							" - Description:        \"" << pd.description << "\"" << endl);
					if (pd.unit != "")
					{
					...
					{
						COUT(
								" - Quantize Step:      " << pd.quantizeStep << endl);
					}
				...
				for (size_t j = 0; j < outputs.size(); ++j)
				{
					...
					COUT(
							" - Description:        \"" << od.description << "\"" << endl);
					if (od.unit != "")
					{
					...
					{
						COUT(
								" - Quantize Step:      " << od.quantizeStep << endl);
					}
					COUT(
							" - Sample Type:        " << (od.sampleType == Plugin::OutputDescriptor::OneSamplePerStep ? "One Sample Per Step" : od.sampleType == Plugin::OutputDescriptor::FixedSampleRate ? "Fixed Sample Rate" : "Variable Sample Rate") << endl);
					if (od.sampleType
							!= Plugin::OutputDescriptor::OneSamplePerStep)
					{
						COUT(
								" - Default Rate:       " << od.sampleRate << endl);
					}
					COUT(
							" - Has Duration:       " << (od.hasDuration ? "Yes" : "No") << endl);
				}
			}
			...
			if (outputs.size() > 1 || verbosity == PluginOutputIds)
			{
				for (size_t j = 0; j < outputs.size(); ++j)
				{
					...
					{
						COUT(
								"         (" << j << ") " << outputs[j].name << ", \"" << outputs[j].identifier << "\"" << endl);
					...
					{
						COUT(
								"vamp:" << key << ":" << outputs[j].identifier << endl);
					}
				}
			}
...
void WinStreamHostWrapper::printPluginCategoryList()
{
	PluginLoader *loader = PluginLoader::getInstance();
	...
	for (size_t i = 0; i < plugins.size(); ++i)
	{
		...
		COUT(
				catstr << key << ":::" << plugin->getName() << ":::" << plugin->getMaker() << ":::" << plugin->getDescription() << std::endl);
	}
}
