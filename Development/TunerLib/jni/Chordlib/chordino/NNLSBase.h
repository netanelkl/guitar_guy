...
enum
{
eChromaMethodsBPS = 3, // bins per semitone
eChromaMethodsOctave = 7,
eChromaMethodsNote = eChromaMethodsOctave * 12 * eChromaMethodsBPS + 2 * (eChromaMethodsBPS/2+1), // a core over all octaves, plus some overlap at top and bottom
eChromaMethodsMIDIBasenote = 45,
eChromaMethodsDict		= eChromaMethodsNote * 84


};

float my_log2( float num );
...
protected:
    ...
    float *m_sfMagnitude;
    float m_arNormaledMagnitude[eChromaMethodsNote];

private:
	void calculateMagnitude(const float* fbuf);
};
