...
protected:
    ...
    float m_dict[eChromaMethodsDict];

private:
	void hmmProcessing(const vector<vector<double> >& chordogram,
			const vector<Vamp::RealTime>& timestamps, FeatureSet& fsOut,
			vector<float>& chordchange, vector<Feature>& oldnotes);
	void simpleChordEstimationProcessing(vector<vector<int> > scoreChordogram,
	...
	int analyzeTunedSpec(FeatureList tunedSpec, FeatureList chromaList,
			int nChord, int count, vector<vector<double> >& chordogram);
};
