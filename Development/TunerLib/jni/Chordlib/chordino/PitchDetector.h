...
public:
	struct DetectionResult
	{
		...
		void print()
		{
			COUT("freq " << dFrequency <<
				 " Prob "<< sfProbability <<
				 " bin " << nBin);
		}
	};

	typedef std::string Method;               // was std::string.
	static const int MIN_THRESHOLD = 80;	// Not related to MIN_FREQ
	static const float MIN_PROBABILITY = 0.8;	// Not related to MIN_FREQ
	static const float MIN_HPS_MAGNITUDE = 10 * 1000 * 1000;	// Not related to MIN_FREQ
	static const float SUBHARMONIC_THRESHOLD = 0.1;	// Not related to MIN_FREQ
	static const float SUBHARMONIC_LOCAL_MAXIMA_WIDTH = 1;	// Not related to MIN_FREQ
	static const int MIN_FREQ = 70;
	static const int HARMONICS = 5;
	...
	PitchDetector(int frameSize, int stepSize, int sampleRate, int nFftSize);
	virtual ~PitchDetector();

	float *getInBuffer(); /**< Get audio data buffer ref */
	DetectionResult getPitch(); /**< Get pitch; use current method */
	...
	static const std::vector<Method> *getMethods(); // was std::vector
	template<typename T>
	static void logToFile(std::string strFilename, int nFileId, T* arData, int nSize);
private:

	float *m_frame;
	double partial();
	DetectionResult autocorrelation();
	DetectionResult hps();
	DetectionResult unwrapPhase(int fBin);
	int getLocalMaxima(int nCentralIndex, int nSearchWindow,
			double array[]);

	class MethodVector: public std::vector<Method>
	{
	public:
		MethodVector();
	};
	...
	double* m_ftMagnitude;
	fftwf_plan m_p1, m_p2, m_pc;
	int m_nHarmonicsDivisor;
	int m_nFftSize;
