...
public class ProfilePictureView extends FrameLayout
{
	...
	private static final boolean	IS_CROPPED_DEFAULT_VALUE		=
																			true;
	private static final String		SUPER_STATE_KEY					=
																			"ProfilePictureView_superState";
	private static final String		PROFILE_ID_KEY					=
																			"ProfilePictureView_profileId";
	private static final String		PRESET_SIZE_KEY					=
																			"ProfilePictureView_presetSize";
	private static final String		IS_CROPPED_KEY					=
																			"ProfilePictureView_isCropped";
	private static final String		BITMAP_KEY						=
																			"ProfilePictureView_bitmap";
	private static final String		BITMAP_WIDTH_KEY				=
																			"ProfilePictureView_width";
	private static final String		BITMAP_HEIGHT_KEY				=
																			"ProfilePictureView_height";
	private static final String		PENDING_REFRESH_KEY				=
																			"ProfilePictureView_refresh";
	...
	private Bitmap					customizedDefaultProfilePicture	=
																			null;
	private final int				nDownloadCounter				=
																			0;

	public interface OnImageChangedEventListener
	{
		public Bitmap onImageChangedEventListener(Bitmap bmp);
	}

	private OnImageChangedEventListener	m_OnImageChangedEventListener;

	public void setOnImageChangedEventListener(OnImageChangedEventListener listener)
	{
		m_OnImageChangedEventListener = listener;
	}
								...
								int heightMeasureSpec)
	{
				...
				&& (params.height == ViewGroup.LayoutParams.WRAP_CONTENT))
		{
			...
			heightMeasureSpec =
					MeasureSpec.makeMeasureSpec(newHeight,
							MeasureSpec.EXACTLY);
			customMeasure = true;
		}
				...
				&& (params.width == ViewGroup.LayoutParams.WRAP_CONTENT))
		{
			...
			widthMeasureSpec =
					MeasureSpec.makeMeasureSpec(newWidth,
							MeasureSpec.EXACTLY);
			customMeasure = true;
		}
	...
	protected Parcelable onSaveInstanceState()
	{
		...
		instanceState.putBoolean(PENDING_REFRESH_KEY,
				lastRequest != null);
	...
	protected void onRestoreInstanceState(Parcelable state)
	{
		...
		else
		{
			...
			presetSizeType =
					instanceState.getInt(PRESET_SIZE_KEY);
			isCropped = instanceState.getBoolean(IS_CROPPED_KEY);
			queryWidth = instanceState.getInt(BITMAP_WIDTH_KEY);
			queryHeight =
					instanceState.getInt(BITMAP_HEIGHT_KEY);

			setImageBitmap((Bitmap) instanceState
					.getParcelable(BITMAP_KEY));
	...
	private void parseAttributes(AttributeSet attrs)
	{
		...
		setPresetSize(a
				.getInt(R.styleable.com_facebook_profile_picture_view_preset_size,
						CUSTOM));
		isCropped =
				a.getBoolean(
	...
	private void setBlankProfilePicture()
	{
		if (customizedDefaultProfilePicture == null)
		{
			...
			setImageBitmap(BitmapFactory.decodeResource(
					getResources(), blankImageResource));
		}
		else
		{
	...
	private void processResponse(ImageResponse response)
	{
		...
		if (response.getRequest() == lastRequest)
		{
			...
			{
				if (m_OnImageChangedEventListener != null)
				{
					responseImage =
							m_OnImageChangedEventListener
									.onImageChangedEventListener(responseImage);
				}
	...
	private boolean updateImageQueryParameters()
	{
		...
		{
			newHeightPx =
					isCropped() ? newWidthPx
							: ImageRequest.UNSPECIFIED_DIMENSION;
		}
		else
		{
			newWidthPx =
					isCropped() ? newHeightPx
							: ImageRequest.UNSPECIFIED_DIMENSION;
		}

		boolean changed =
				(newWidthPx != queryWidth)
	...
	private int getPresetSizeInPixels(boolean forcePreset)
	{
		...
		switch (presetSizeType)
		{
			case SMALL:
				dimensionId =
						R.dimen.com_facebook_profilepictureview_preset_size_small;
				break;
			case NORMAL:
				dimensionId =
						R.dimen.com_facebook_profilepictureview_preset_size_normal;
				break;
			case LARGE:
				dimensionId =
						R.dimen.com_facebook_profilepictureview_preset_size_large;
				break;
			case CUSTOM:
				if (!forcePreset)
				{
